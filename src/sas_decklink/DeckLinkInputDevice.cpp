//File modified by Murilo M. Marinho 2021
...
bool DeckLinkInputDevice::GetLatestFrame(IDeckLinkVideoFrame** frame)
{
	std::unique_lock<std::mutex> lock(m_deckLinkInputMutex, std::try_to_lock); 
	if (lock.owns_lock())
	{
		if (!m_videoFrameQueue.empty())
		{
			*frame = m_videoFrameQueue.back();
			return true;
		}
	}
	return false;
...
void DeckLinkInputDevice::CleanQueue()
{
	std::unique_lock<std::mutex> lock(m_deckLinkInputMutex, std::try_to_lock);
	if (lock.owns_lock())
	{
